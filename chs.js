...
var cnItems = {
    ...
    "Specify planes to auto-purchase upgrades from by dragging their portals to the booster. You can only select a single plane to start, but that can be increased using an action.": "通过将其传送门拖动到助推器来指定要自动购买升级的飞机。 您只能选择一个平面开始，但可以使用一个动作来增加。",
    "Specify planes to boost by dragging their portals to the booster. You can only select a single plane to start, but that can be increased using an action.": "通过将它们的传送门拖到助推器来指定要增强的飞机。 您只能选择一个平面开始，但可以使用一个动作来增加。",
    ...
    "+1% plane's resource gain per minute active": "+1% 飞机每分钟资源增益",
    "+resource mults": "+资源倍数",
    "10x cost": "10 倍成本",
    "Aphadynpulse": "失音脉冲",
    ...
    "Spawn dirt-tier portal - 1.00e9 energy": "产生 灰尘位面 传送门 - 1.00e9 能量",
